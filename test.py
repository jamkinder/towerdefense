    ...
    def update(self, *args):
        ...
        if event.type == KEYDOWN:
            if event.key == K_d and not pygame.sprite.spritecollideany(self, vertical_borders, ):
                if (thislevel[(self.rect.x // 50) + 1][self.rect.y // 50] == '.'
                        or thislevel[(self.rect.x // 50) + 1][self.rect.y // 50] == '@'):
            ...
            if event.key == K_w and not pygame.sprite.spritecollideany(self, horizontal_borders):
                if (thislevel[(self.rect.x // 50)][(self.rect.y // 50) - 1] == '.'
                        or thislevel[(self.rect.x // 50)][(self.rect.y // 50) - 1] == '@'):
